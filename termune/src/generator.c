...
int _generator_connect_rooms(dungeon_data *dungeon)
{
    ...
    for (i = 0; i < dungeon->num_rooms; i++)
    {
        ...
        for (j = 0; j < path->size; j++)
        {
            ...
            if (dungeon->cell_types[y][x] == CELL_ROCK)
            {
                dungeon->cell_types[y][x] = CELL_CORRIDOR;
                dungeon->cell_hardness[y][x] = 0;
            }
        }
        vector_destroy(path);
    }
...
int generator_generate_dungeon(dungeon_data *dungeon, const generator_parameters *params)
{
    ...
    for (i = 0; i < num_stairs; i++)
    {
        ...
        } while (dungeon->cell_types[y][x] == CELL_ROCK);
        dungeon->cell_types[y][x] = stair_direction ? CELL_STAIR_DOWN : CELL_STAIR_UP;
        dungeon->cell_hardness[y][x] = 0;
        stair_direction ^= 1;
    }
...
int generator_apply_room(dungeon_data *dungeon, const dungeon_room_data *room_data)
{
    ...
    for (dx = 0; dx < room_data->width; dx++)
    {
        ...
        {
            dungeon->cell_types[y + dy][x + dx] = CELL_ROOM;
            dungeon->cell_hardness[y + dy][x + dx] = 0;
        }
    }
...
int generator_undo_room(dungeon_data *dungeon, const dungeon_room_data *room_data)
{
    ...
    for (dx = 0; dx < room_data->width; dx++)
    {
        for (dy = 0; dy < room_data->height; dy++)
        {
            dungeon->cell_types[y + dy][x + dx] = CELL_ROCK;
        }
    }
    ...
    if (params->rock_hardness_noise_amount > 0.f)
    {
        ...
        for (y = 0; y < DUNGEON_HEIGHT; y++)
        {
            for (x = 0; x < DUNGEON_WIDTH; x++)
            {
                ...
                noise = (int32_t)(layered_noise_perlin(x + x_off, y + y_off, params->rock_hardness_noise_amount, 0.15f, 8, 0.5f, 2.f));

                hardness[i] = VALUE_CLAMP((int32_t)(hardness[i]) + noise, 1, 254);
                i++;
            }
        }
    }
    ...
    for (y = 0; y < DUNGEON_HEIGHT; y++)
    {
        for (x = 0; x < DUNGEON_WIDTH; x++)
        {
            if (dungeon->cell_types[y][x] == CELL_ROCK)
            {
                ...
                {
                    dungeon->cell_hardness[y][x] = 255;
                }
                else
                {
                    dungeon->cell_hardness[y][x] = hardness[i];
                }
            }
            else
            {
